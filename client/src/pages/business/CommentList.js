...
const CommentList = ({ itemData, setItemData, businessState }) => {
  ...
  const handleDelete = (commentId) => {
    fetch(
      `/businesses/${business.id}/menu/categories/${itemData.category_id}/items/${itemData.id}/comments/${commentId}`,
      {
        method: "DELETE",
      }
    )
      .then((res) => {
        if (res.ok) {
          setItemData({
            ...itemData,
            comments: itemData.comments.filter(
  ...
  const renderComments = itemData.comments
        ...
        <React.Fragment key={comment.id}>
          <ListItem alignItems="flex-start" key={comment.id}>
            ...
            <ListItemText
              ...
              secondary={`Posted ${timeAgo(
                comment.created_at
              )} by ${commentAuthor}`}
            />
            {comment.isAuthor || hasAccess ? (
